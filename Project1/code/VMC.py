...
class VMC:
    ...
    def QuantumForce(self, r):
        if r.shape[1] == 3:
            r[:, 2] *= np.sqrt(self.beta)
        qforce = -4 * self.alpha * r
        return qforce
    ...
    def local_energy(self, r, dim):
        ...
        if self.N_particles == 1:
            laplacian_sum += (
                4 * self.alpha**2 * np.sum(r**2) - 6 * self.alpha
            )
            # Local energy
            EL = (
        ...
        if False: #dim == 1 or dim == 2:
            ...
            r_diff = np.delete(
                r_diff.reshape(N**2, dim),
                np.arange(N**2, step=N + 1),
                axis=0,
            ).reshape(N, N - 1, dim)
            r_norm = np.delete(
            ...
            # Store as class variables for use in other methods 
            self.r_diff = r_diff
            self.r_norm = r_norm
            ...
            laplacian_sum = (
                ...
                * alpha
                * a
                * np.sum(r @ np.sum(r_diff, axis=(1)).T, axis=1)
                / np.sum(r_norm**3 - a * r_norm**2, axis=-1)
                + np.sum(
                    (a**2 - 2 * a * r_norm) / (r_norm**2 - a * r_norm) ** 2
                    + 2 * a / (r_norm**3 - a * r_norm**2),
                    axis=-1,
                )
    ...
    def SGD(self, r, lr):
        if r.shape[0] == 1:
            for i in range(1000):
                grad_EL = -self.h_bar**2 / (2 * self.m) * (
                    8 * self.alpha * np.sum(r**2) - 6 
                )
                self.alpha -= lr * grad_EL
        else:
            for i in range(1000):
                grad_EL = -self.h_bar**2 / (2 * self.m) * np.sum(
                8 * self.alpha * np.einsum("ij,ij->i", r, r) - 6 
                - 4 * self.a * np.sum(r @ np.sum(self.r_diff, axis=(1)).T, axis=1)
                / np.sum(self.r_norm**3 - self.a * self.r_norm**2, axis=-1)
                )
                print(grad_EL)
                print(self.alpha)
                self.alpha += lr * grad_EL 
    ...
    def MC_Sampling(
        ...
        for ia in range(MaxVariations):
            ...
            if Importance_sampling:
                QuantumForceOld = self.QuantumForce(PositionOld)
                ...
                for i in range(self.N_particles):
                    for j in range(Dimension):
                        if Importance_sampling:
                            PositionNew[i, j] = (
                                PositionOld[i, j]
                                + normalvariate(0.0, 1.0) * np.sqrt(StepSize)
                                + D * StepSize * QuantumForceOld[i, j]
                            )
                            wfnew = self.wavefunction(PositionNew)
                            QuantumForceNew = self.QuantumForce(
                                PositionNew
                            )
                            GreensFunction = 0.0
                            for j in range(Dimension):
                            ...
                            ProbabilityRatio = (
                                GreensFunction * wfnew**2 / wfold**2
                            )
                            if random() <= ProbabilityRatio:
                                for j in range(Dimension):
                                    PositionOld[i, j] = PositionNew[i, j]
                                    QuantumForceOld[i, j] = QuantumForceNew[
                                        i, j
                                    ]
                                wfold = wfnew
